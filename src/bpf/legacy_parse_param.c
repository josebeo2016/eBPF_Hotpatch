...
/*
 * Attach to the "legacy_parse_param" 
 */
...
int kprobe__legacy_parse_param(struct pt_regs *ctx, struct fs_context *fc, struct fs_parameter *param) {
    ...
    struct legacy_fs_context *ctx_ = fc->fs_private;
	unsigned int size = ctx_->data_size;
	size_t len = 0;
